...
void RobotDataCollectionTask(flexiv::Robot* robot, 
                             ...
                             flexiv::PlanInfo* planInfo)
{
    while (true) {
        ...
        if (stackFlag == true){
            for (int i=0; i<7; i++){
                outdata[i] = g_printData.tcp_pose[i];
            }
            for (int i=0; i<6; i++){
                outdata[i+7] = g_printData.raw_sensor_data[i];
            }
            for (int i=0; i<3; i++){
                outdata[i+13] = g_printData.flangePose[i];
            }
                    ...
                    std::string filePath)
{
    ...
    std::thread robthread(std::bind(std::bind(RobotDataCollectionTask, robot, robotStates, planInfo)));
...
int main(int argc, char* argv[])
{
    ...
    std::string localIP = "192.168.2.101";
